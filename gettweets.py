import ConfigParser, urllib, random, base64, hmac, binascii, time
import collections, hashlib, os, multiprocessing, datetime
...
def newRequest():
    ...
    return "curl --get '{0}' --header 'Authorization: {1}' --verbose".format(url, headers['Authorization'])
...
def main():
    ...
    while(True):
        ...
        p = multiprocessing.Process(target=gettweets, name="gettweets", args=(newRequest(),))
        p.start()

        # Wait 10 seconds for for tweets                                                                        
        time.sleep(60)
        ...
        os.system('mv tweets.json /root/data/tweets' + st + '.json')
